...
//import { vueAbonnementListe } from "./class_abonnement_liste";
...
type TAbonnementEditForm = { 
    ...
    areaCommentaire:HTMLInputElement,
...
class VueAbonnementEdit{
    ...
    initMsgErreur():void { 
        ...
        verifNum(valeur : string):void {
            ...
            if (chaine.length > 0) {
                ...
                if (! chaine.match(/^([a-zA-Z0-9]+)$/)) { this._erreur.edtAbonnementNum.statut = 'inconnu'; }
    ...
    detailAdherent(valeur : string):void {
                const err = this._erreur.edtAbonnementNum
                ...
                const chaine : string = valeur.trim();
                if (chaine.length > 0) {
                const adh : UnAdherent = lesAdhs.byAdhNum(chaine);
                if (adh.adhNum !== "") { 
                adh2.textContent
        ...
        retourClick(fichierHTML:string):void {
